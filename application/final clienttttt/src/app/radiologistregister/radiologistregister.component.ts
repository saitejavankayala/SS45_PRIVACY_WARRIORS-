...
@Component({
  selector: 'app-radiologistregister',
  templateUrl: './radiologistregister.component.html',
  styleUrls: ['./radiologistregister.component.scss']
})
export class RadiologistregisterComponent implements OnInit {
  ...
  doctoronclick(){
    this.loading = true;
    this.authService.rabody={
      aadhar:this.model.rauserid,
      password:this.model.rapassword,
    ...
    this.authService.registerra(this.model).subscribe(data => {
      if(data=='true'){
  ...
  this.router.navigate(['/radiologistregister']);
        }
    }, error => {
      this.loading = false;
